...
int parse_token(const char* text, const char* delims, OnTokenFunc on_token,
                void* ctx)
{
    enum _State {
        STAT_INIT,
...
{
    assert(parse_token("it is used to token segmentation. is it easy?", " .?",
                       on_token, NULL) == 9);
    assert(parse_token("/backup/tools/jdk1.5.0_18/bin/:/usr/lib/qt-3.3/bin:/usr/kerberos/bin:/backup/tools/jdk1.5.0_18/bin/:/usr/lib/ccache:/usr/local/bin:/usr/bin:/bin:/home/lixianjing/bin",
                       ":", on_token, NULL) == 9);
