...
static void xml_parser_parse_attrs(XmlParser* thiz, char end_char)
{
    ...
    for (; *thiz->read_ptr != '\0' &&
         thiz->attrs_nr < MAX_ATTR_NR; thiz->read_ptr++) {
        ...
        case STAT_KEY: {
                if (c == '=') {
                    thiz->attrs[thiz->attrs_nr++] = (char*)xml_parser_strdup(thiz, start,
                                                    thiz->read_ptr - start);
...
static void xml_parser_parse_start_tag(XmlParser* thiz)
{
    ...
    xml_builder_on_start_element(thiz->builder, tag_name,
                                 (const char**)thiz->attrs);
